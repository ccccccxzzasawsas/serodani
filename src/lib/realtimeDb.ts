...
// მთავარი გვერდის სურათების მენეჯმენტი Realtime Database-ში

// სექციის კონტენტის შენახვა Realtime Database-ში
export const saveSectionToRealtime = async (section: string, content: any): Promise<void> => {
  try {
    await set(ref(rtdb, `sections/${section}`), {
      ...content,
      updatedAt: new Date().toISOString()
    });
    return;
  } catch (error) {
    console.error(`Error saving ${section} to realtime database:`, error);
    throw error;
  }
};

// სექციის კონტენტის წაკითხვა Realtime Database-დან
export const getSectionFromRealtime = async (section: string): Promise<any | null> => {
  try {
    const snapshot = await get(ref(rtdb, `sections/${section}`));
    ...
    console.error(`Error getting ${section} from realtime database:`, error);
    throw error;
  }
};

// სექციის კონტენტის მოსმენა რეალურ დროში
export const listenToSection = (section: string, callback: (content: any | null) => void) => {
  const sectionRef = ref(rtdb, `sections/${section}`);
  ...
  onValue(sectionRef, handleSnapshot);
  
  // დავაბრუნოთ ფუნქცია, რომელიც გამორთავს მოსმენას
  return () => off(sectionRef);
};

// მთავარი გვერდის ჰერო სურათის განახლება
export const updateHeroSectionRealtime = async (imageUrl: string): Promise<void> => {
  await saveSectionToRealtime("hero", { imageUrl });
};

// სლაიდერის სურათების განახლება
export const updateSliderImagesRealtime = async (imageUrls: string[]): Promise<void> => {
  await saveSectionToRealtime("slider", { imageUrls });
};

// სთორის სურათების განახლება
export const updateStoryImagesRealtime = async (imageUrls: string[]): Promise<void> => {
  await saveSectionToRealtime("story", { imageUrls });
};

// დიდი სურათის განახლება
export const updateLargePhotoRealtime = async (imageUrl: string): Promise<void> => {
  await saveSectionToRealtime("largePhoto", { imageUrl });
};

// გესთის რევიუს სურათის განახლება
export const updateGuestReviewImageRealtime = async (imageUrl: string): Promise<void> => {
  await saveSectionToRealtime("guestReview", { imageUrl });
}; 